...
def SummaryTable(gridX,gridY,building_use,facadeType,floorToCeiling,designTypology):
    ...
    gridXdb = min(gridX,gridY)
    gridYdb = max(gridX,gridY)
    buildingUseDb = building_use
    ...
    #Outputs
    facadeLoad = (float(get_value_by_description(facadeType,facadeTable,connection_string))*floorToCeiling)
    if(facadeLoad<10):
        facadeLoadRC = 0
    else:
