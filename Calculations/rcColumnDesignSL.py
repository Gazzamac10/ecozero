...
#Whole thing turned into a definition for streamlit

def rcColumnDesign(summary_df,gk,qk,concMix,storeys,floorToCeiling,perReinforcement,building_use,bayX,bayY,gridX,gridY):
    columns = definitions.ColumnNumbers(bayX,bayY)
    columnsInternal = columns[0]
    columnCorners = columns[1]
    columnEdges  =columns[2]
    print(columnsInternal)
    #RCColumns
    ...
    planArea = definitions.floorArea(bayX,bayY,gridX,gridY)
    ...
    return rcColumnLoads,InternalRcColumn,EdgeRcColumn,CornerRcColumn,ColumnSummary,pileLoads
